    ...
    S2: AsRef<str>,
{
    ...
    // OmniShell patch: disable shrs_job terminal job-control handoff.
    // Upstream enables this unconditionally and calls tcsetpgrp() from the
    // child pre_exec hook. In OmniShell this can stop the child before exec,
    // leaving the parent waiting forever for ordinary commands such as `w`.
    // OmniShell still executes commands directly (no sh -c); interactive
    // foreground commands and pipelines use OmniShell-owned execution paths.
    let job_control_is_enabled = false;
